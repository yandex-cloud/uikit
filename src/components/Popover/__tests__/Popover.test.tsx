...
test('Can be opened/closed on hover/unhover', () => {
    ...
    user.hover(popoverTrigger);
    waitForTooltipOpenedStateChange(true);

    checkIfPopoverOpened();

    user.unhover(popoverTrigger);
...
test('Renders with buttons', () => {
    ...
    const actionButton = screen.getByText(actionButtonConfig.text);
    const cancelButton = screen.getByText(cancelButtonConfig.text);
